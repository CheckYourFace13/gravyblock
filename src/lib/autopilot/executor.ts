...
/**
 * One-time re-baseline sweep for house accounts (Boating Chicago, League
 * Pour, etc. — accountType='house', the accounts /proof shows as canaries
 * exercising the same automation path a paying customer gets). Brings each
 * one's NEXT pending recurring-refresh job forward to "due now" instead of
 * waiting out its normal cadence, so /proof reflects a fresh recompute under
 * the current (visibility-v2) scoring methodology rather than whatever
 * score happened to be current from an earlier formula. If a house account
 * has no pending job at all, schedules one immediately instead.
 */
export async function reinitializeHouseAccountBaselines(): Promise<{ broughtForward: number; scheduled: number; checked: number }> {
  const db = getDb();
  if (!db) return { broughtForward: 0, scheduled: 0, checked: 0 };
  ...
  if (houseAccounts.length === 0) return { broughtForward: 0, scheduled: 0, checked: 0 };
  ...
  for (const biz of houseAccounts) {
    ...
    if (pending) {
      await db.update(jobs).set({ runAfter: new Date() }).where(eq(jobs.id, pending.id));
      broughtForward++;
      continue;
    }
    ...
    const tier = normalizePlanTierFromDb(biz.planTier);
    if (tier === "free") continue;
    const result = await scheduleRecurringSnapshotJob({ businessId: biz.id, runAfterMs: 0, type: recurringJobTypeForPlan(tier) });
    if (!result.skipped) scheduled++;
  }

  return { broughtForward, scheduled, checked: houseAccounts.length };
}
