...
function ReviewCard({ review }: { review: Review }) {
  ...
  const ratingColor =
    review.rating >= 4 ? "bg-green-50 border-green-100" : review.rating === 3 ? "bg-yellow-50 border-yellow-100" : "bg-red-50 border-red-100";

  return (
    <li className={`rounded-xl border p-4 ${ratingColor}`}>
        ...
        <div className="mt-3 border-t border-zinc-200/60 pt-3">
          ...
          {expanded ? (
            <div className="mt-2">
              ...
              <p className="mt-1 text-[10px] text-zinc-400">Copy and paste into Google Maps or Yelp to reply.</p>
            </div>
          ) : null}
        </div>
      ) : null}
  ...
  return (
    <section className="rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm">
      <div className="flex items-start justify-between gap-4">
        <div>
          <h2 className="text-lg font-semibold text-zinc-900">Review monitoring</h2>
          <p className="mt-1 text-sm text-zinc-600">
            Latest reviews from Google. AI-drafted replies you can copy and paste.
          </p>
        </div>
        {avgRating !== null ? (
          <div className="shrink-0 text-right">
            ...
            <p className="mt-0.5 text-xs text-zinc-500">{reviews.length} reviews synced</p>
          </div>
        ) : null}
      </div>
      {newCount > 0 ? (
        <div className="mt-3 rounded-lg bg-yellow-50 border border-yellow-100 px-3 py-2 text-sm font-medium text-yellow-800">
          {newCount} new review{newCount === 1 ? "" : "s"} since last check
        </div>
      ) : null}
